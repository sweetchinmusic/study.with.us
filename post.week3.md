...
[2번째 과제](https://github.com/sweetchinmusic/study.with.us/blob/main/post.week2.md)에서 학습했듯이 타입 프로모션 때문입니다. 큰 타입과 작은 타입을 연산을 했을때 결과의 타입은 큰 타입으로 나오게 됩니다.
...
result : false, n1:10, n2:0
result : true, n1:20, n2:0
```

* 위의 각 두 연산에서는 n1과 n2 값을 10씩 증가시키는 연산이 포함되어있다. 따라서 n1, n2 모두 값이 20이 되어야 하는데, 결과를 보면 n2는 증가하지 않았다.
* 이유는 위에 설명한 SCE 때문이다.
```
(n1 += 10) < 0 && (n2 += 10) > 0
```
* 위 코드에서 `&&`를 기준으로 좌측 관계 연산자의 결과가 `false`인데, `&&`연산의 경우 하나라도 `false`이면 이미 결과 값은 `false`이기 때문에 우측 연산을 생략한다.
```
(n1 += 10) > 0 || (n1 += 10) > 0
```
* 위 코드에서 `||`를 기준으로 좌측 관계 연산의 결과가 `true`인데, `||`연산의 경우 하나라도 `true`이면 이미 결과는 `true`이다. 이런 경우 우측 연산을 생략한다.
...
다음과 같이 `Number`, `Comparable` 인터페이스 두 개의 인터페이스를 구현한 `NewInteger`, `NewDouble`이라는 클래스가 있으면 두 인터페이스의 함수를 모두 사용할 수 있게됩니다. 
사용방법은 디컴파일한 class 파일을 보겠습니다.
...
보시는 것과 같이 하나의 타입으로 받은 후 함수를 쓸 때마다 해당 함수가 있는 인터페이스(혹은 클래스)로 강제 형변환을 하게 됩니다. 하지만 이 경우 첫 번째로 출력한 `byteValue` 메서드는 문제가 없지만 `compareTo` 메서드의 경우 제네릭 타입의 불일치로 에러가 발생하게 됩니다.
